...
struct BMP {
  ...
  /*
    ...
    */
    void transform_matrix_to_greyscale() {
        uint32_t channels = bmp_info_header.bit_count / 8;
        uint32_t A{ 0 }, R{ 0 }, G{ 0 }, B{ 0 }, grey{ 0 };
        //mtx_grey.lock();
        for (uint32_t row = 0; row < bmp_info_header.height; row++) {
            for (uint32_t column = 0; column < bmp_info_header.width; column++) {
                R = data[channels * (row * bmp_info_header.width + column) + 0];
                G = data[channels * (row * bmp_info_header.width + column) + 1];
                B = data[channels * (row * bmp_info_header.width + column) + 2];

                grey = ((0.3 * R) + (0.59 * G) + (0.11 * B));

                data[channels * (row * bmp_info_header.width + column) + 0] = grey;
                data[channels * (row * bmp_info_header.width + column) + 1] = grey;
                data[channels * (row * bmp_info_header.width + column) + 2] = grey;
            }
        }
        //global_row_greyscale_threads += 1;
        //mtx_grey.unlock();
